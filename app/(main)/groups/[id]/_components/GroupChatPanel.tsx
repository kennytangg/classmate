...
interface GroupChatPanelProps {
  groupId: string
  onGroupLoaded?: (group: LoadedGroup) => void
}

export function GroupChatPanel({ groupId, onGroupLoaded }: GroupChatPanelProps) {
  ...
  const loadMessages = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        if (data.group && !groupLoadedRef.current) {
          groupLoadedRef.current = true
          onGroupLoaded?.(data.group)
        }
  ...
  return (
    <>
      ...
      <div
        ...
        className="bg-background flex-1 space-y-4 overflow-y-auto p-4"
      >
        {loading && (
      ...
      <div className="px-4 py-3">
        ...
        <div className="border-border bg-muted rounded-2xl border shadow-sm transition-shadow hover:shadow-md">
          {/* Pending file preview — inside the card */}
          {pendingFile && (
              ...
              {pendingFile.previewUrl ? (
                <div className="relative inline-block">
                  ...
                  <button
                    ...
                    className="bg-background border-border hover:bg-card absolute -top-1.5 -right-1.5 rounded-full border p-0.5 shadow-sm transition-colors"
          ...
          <div className="flex items-center gap-2 p-2">
            <Button
              ...
              className="text-muted-foreground hover:text-foreground hover:bg-accent ml-1 h-8 w-8 shrink-0 rounded-xl"
            ...
            <Button
              ...
              className="bg-primary text-primary-foreground shadow-primary/20 hover:bg-primary/90 h-8 w-8 shrink-0 rounded-xl shadow-sm transition-colors disabled:cursor-not-allowed disabled:opacity-40"
            >
              {uploading || sending ? (
