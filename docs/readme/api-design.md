...
| Method       | Endpoint                          | Description                              | Auth Required |
| :----------- | :-------------------------------- | :--------------------------------------- | :------------ |
| GET          | `/api/forums/posts`               | List posts (paginated, searchable)       | Yes           |
| POST         | `/api/forums/posts`               | Create post (AI moderation applied)      | Yes           |
| GET          | `/api/forums/posts/[id]`          | Get post details (increments view count) | No            |
| PATCH        | `/api/forums/posts/[id]`          | Update post (owner only)                 | Yes           |
...
| PATCH/DELETE | `/api/forums/replies/[id]`        | Edit (owner) / delete (owner or mod)     | Yes           |
...
| POST     | `/api/messages/conversations/[userId]/read` | Mark messages as read | Yes           |
...
| POST/DELETE     | `/api/study-groups/[groupId]/join`        | Join / leave a study group  | Yes           |
| GET/POST        | `/api/study-groups/[groupId]/messages`    | Group chat read/send        | Yes           |
| POST            | `/api/study-groups/[groupId]/invite-code` | Generate group invite code  | Yes           |
| POST            | `/api/study-groups/join-by-code`          | Join a group by invite code | Yes           |
...
| GET              | `/api/materials/[id]/preview`  | Inline file preview                    | Yes           |
...
| PATCH/DELETE | `/api/events/[id]` | Update/delete event | Yes           |

### Moderation (5 endpoints)

| Method | Endpoint                  | Description                         | Auth Required |
| :----- | :------------------------ | :---------------------------------- | :------------ |
| POST   | `/api/moderation`         | Content moderation check (internal) | Yes           |
...
### Users & Notifications (5 endpoints)
...
| GET/PATCH/DELETE | `/api/notifications` | List / mark read / delete notifications | Yes           |

### Misc (3 endpoints)

| Method | Endpoint      | Description                          | Auth Required |
| :----- | :------------ | :----------------------------------- | :------------ |
| POST   | `/api/upload` | Upload a file attachment (max 10 MB) | Yes           |
| GET    | `/api/health` | Health check (DB connectivity)       | No            |
| GET    | `/api/docs`   | OpenAPI 3.0.3 spec (JSON)            | No            |

---

## 6.2 API Documentation

**Swagger UI:** Available at `/docs` (live app) and `http://localhost:3000/docs` (local dev)
